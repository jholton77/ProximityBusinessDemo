...
-(BOOL) insert:(NSString *)key : (NSData *)data
{
    ...
    if(errSecSuccess != status) {
        NSLog(@"Unable add item with key =%@ error:%@",key,NSStringFromOSStatus(status));
    }
...
-(NSData*) find:(NSString*)key
{
    ...
    if( status != errSecSuccess) {
        //NSLog(@"Unable to fetch item for key %@ with error:%@",key,NSStringFromOSStatus(status));
        return nil;
    }
...
-(BOOL) update:(NSString*)key :(NSData*) data
{
    ...
    if(errSecSuccess != status) {
        //NSLog(@"Unable add update with key =%@ error:%@",key,NSStringFromOSStatus(status));
    }
    return (errSecSuccess == status);
...
-(BOOL) remove: (NSString*)key
{
    ...
    if( status != errSecSuccess) {
        //NSLog(@"Unable to remove item for key %@ with error:%@",key,NSStringFromOSStatus(status));
        return NO;
    }
    return  YES;
}
